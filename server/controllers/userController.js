...
updateUser = async (req, res) => {
    ...
    User.findOne({ _id: req.params.id }, (err, flight) => {
        if (err) {
...
deleteUser = async (req, res) => {
    await User.findByIdAndDelete({ _id: req.params.id }, (err, flight) => {
        if (err) {
...
createUser = (req, res) => {
    User.create(req.body)
        .then((user) => res.json({ msg: "User added successfully", data: req.body }))
        .catch((err) =>
...
getUser = (req, res) => {
    User.find()
        ...
        .catch((err) =>
            res.status(404).json({ nousersfound: "No User found" })
        );
}
